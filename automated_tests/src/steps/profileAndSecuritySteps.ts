...
When('I enter 6 digit MFA code generated by authenticator app', { timeout: 120000 }, async function(this: CustomWorld) {
    if (!this.secretCode) {
        console.error('❌ Secret code not available. Please ensure the MFA setup step is run before this step.');
        console.error('💡 Add "When I navigate to Security and Privacy page" and "And I setup authenticator app through API" steps before this step.');
    ...
    try {
        ...
        // Approach 3: Try direct element search with optimized selectors
        if (!mfaEntered) {
            try {
                console.log('🔍 Trying direct MFA input element search...');
                
                // Prioritized selectors - most likely to work first
                const mfaSelectors = [
                ...
                for (const selector of mfaSelectors) {
                    try {
                        ...
                        if (await mfaInput?.isVisible({ timeout: 1000 })) { // Reduced timeout
                ...
                if (mfaInput && await mfaInput.isVisible()) {
                    ...
                    try {
                        ...
                        if (await confirmButton?.isVisible({ timeout: 1000 })) { // Reduced timeout
        ...
        // Wait for the form to process with reduced timeout
        await this.page?.waitForTimeout(1000); // Reduced from 2000ms
        
        // Check if MFA was successful by verifying dashboard access
        console.log('🔍 Checking if MFA was successful by verifying dashboard access...');
        ...
        try {
            ...
            if (isDashboardVisible) {
                console.log('✅ MFA successful - Dashboard is visible');
            } else {
                console.log('❌ MFA failed - Dashboard not visible, checking for error messages...');
                
                // Quick error check with reduced selectors
                ...
                for (const selector of errorSelectors) {
                    try {
                        ...
                        if (await errorElement?.isVisible({ timeout: 500 })) { // Reduced timeout
                ...
                if (!errorFound) {
                    console.log('⚠️ No specific error message found, but dashboard not visible');
                }
                
                throw new Error('MFA failed - Dashboard not accessible after code entry');
            }
        } catch (dashboardCheckError) {
            console.log('⚠️ Error checking dashboard visibility:', dashboardCheckError);
            throw new Error('MFA failed - Could not verify dashboard access');
        }
    } catch (error) {
        console.log('❌ MFA entry failed:', error);
        // Take a screenshot for debugging
        await this.page?.screenshot({ path: `mfa-error-${Date.now()}.png`, fullPage: true });
        throw error;
    }
});
...
When('I enter 6 digit MFA code with retry if dashboard not accessible', { timeout: 120000 }, async function(this: CustomWorld) {
    if (!this.secretCode) {
        console.error('❌ Secret code not available. Please ensure the MFA setup step is run before this step.');
        console.error('💡 Add "When I navigate to Security and Privacy page" and "And I setup authenticator app through API" steps before this step.');
        throw new Error('Secret code not available. Please run the MFA setup step first.');
    ...
    console.log('🔢 Entering 6-digit MFA code with retry logic if dashboard not accessible...');
    
    const maxRetries = 3;
    const retryDelay = 2000; // 2 seconds between retries
    
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        console.log(`🔄 MFA attempt ${attempt}/${maxRetries}`);
        ...
        try {
            ...
            // Take a screenshot for debugging
            await this.page?.screenshot({ path: `mfa-page-before-input-attempt-${attempt}-${Date.now()}.png`, fullPage: true });
            console.log(`📸 Screenshot taken before MFA input attempt ${attempt}`);
            ...
            if (!mfaEntered) {
                try {
                    ...
                    console.log('🏠 Using dashboard page MFA input for MFA setup...');
                    await this.dashboardPage.enterMFAOTP(this.secretCode);
                    mfaEntered = true;
                } catch (error) {
                    console.log('⚠️ Dashboard page MFA input failed:', error);
            ...
            // Approach 3: Direct element search as fallback
            if (!mfaEntered) {
                try {
                    console.log('🔍 Trying direct element search for MFA input...');
            ...
            if (!mfaEntered) {
                throw new Error('MFA input field not found after trying all approaches');
            }
            ...
            await this.page?.screenshot({ path: `mfa-after-input-attempt-${attempt}-${Date.now()}.png`, fullPage: true });
            console.log(`📸 Screenshot taken after MFA input attempt ${attempt}`);
            
            // Check if MFA was successful by verifying dashboard access
            console.log('🔍 Checking if MFA was successful by verifying dashboard access...');
            await this.page?.waitForTimeout(3000); // Wait for potential redirect
            ...
            try {
                ...
                if (isDashboardVisible) {
                    ...
                    if (attempt < maxRetries) {
                        ...
                        throw new Error('MFA failed after all retry attempts - Dashboard not accessible');
                    }
                }
            } catch (dashboardCheckError) {
                console.log('⚠️ Error checking dashboard visibility:', dashboardCheckError);
                ...
                if (attempt < maxRetries) {
                    ...
                    throw new Error('MFA failed after all retry attempts - Could not verify dashboard access');
                }
            }
            
        } catch (error) {
            console.log(`❌ MFA attempt ${attempt} failed:`, error);
            ...
            if (attempt < maxRetries) {
                ...
                await this.page?.screenshot({ path: `mfa-final-failure-${Date.now()}.png`, fullPage: true });
                console.log('📸 Final failure screenshot taken');
                ...
                throw new Error(`MFA failed after ${maxRetries} attempts: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    }
});
