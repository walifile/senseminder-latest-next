...
export const supportTabIconClass = (icon: string, sizeClass?: string) =>
  cn(
    "bg-current",
    sizeClass || "w-4 h-4",
    "[mask-image:url('" + icon + "')] [mask-size:contain] [mask-repeat:no-repeat] [mask-position:center]",
    "[-webkit-mask-image:url('" + icon + "')] [-webkit-mask-size:contain] [-webkit-mask-repeat:no-repeat] [-webkit-mask-position:center]"
  );
