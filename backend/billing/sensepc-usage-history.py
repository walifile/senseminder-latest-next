...
resource_table = dynamodb.Table('SmartPCResourceTracking')

def enrich_items_with_status(items):
    if not items:
        return items
    
    try:
        # Enrich billing items with status
        enriched_items = []
        for item in items:

            # Check if systemStatus field exists
            if 'systemStatus' in item:
                item['status'] = item.get('systemStatus')
            else:
                # Determine status based on instanceCost and billingPlan
                instance_cost = item.get('instanceCost')
                billing_plan = item.get('billingPlan')

                try:
                    cost_val = Decimal(instance_cost)
                except (TypeError, ValueError):
                    cost_val = 0.0
                
                if cost_val <= 0 and billing_plan == "hourly":
        ...
        print(f"Error enriching items with status: {str(e)}")
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if not is_action_allowed(user_role):
            return _response(403, {"message": "Members are not allowed to retrieve Usage history"})
        ...
        if last_evaluated_key_param:
            try:
                ...
                required_fields = ['instanceId', 'userId', 'timestamp']
                if not all(field in last_evaluated_key for field in required_fields):
                    ...
                    return _response(400, {
                        ...
                        'message': 'lastEvaluatedKey must contain instanceId, userId, and timestamp'
                    })
        ...
        if items:
            items = enrich_items_with_status(items)
