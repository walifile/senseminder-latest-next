...
def lambda_handler(event, context):
    ...
    try:
        ...
        wallet_data = wallet_response.get("Item") or {}
        ...
        response_data = {
            "promoBalance": float(wallet_data.get("promoBalance", 0)),
            "cashback": float(wallet_data.get("cashback", 0)),
            "balance": float(wallet_data.get("balance", 0)),
            "lastRecharge": {
                "amount": last_recharge_amount,
                "timestamp": last_recharge_timestamp
            },
            "autoRecharge": wallet_data.get("autoRecharge", False),
            "autoRechargeAmount": float(wallet_data.get("autoRechargeAmount", 0))
        }    
        print(f"wallet Balance: {response_data}")
